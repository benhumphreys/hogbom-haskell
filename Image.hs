...
readVecFloat f = do
    ...
    where listOfFloat32le input
            | B.null input = []
            | otherwise = let (val, rest, _) = B.runGetState getFloat32le input 0
                        in val : listOfFloat32le rest

-- | Writes a vector to a file
writeVecFloat :: FilePath -> V.Vector Float -> IO ()
writeVecFloat f xs = B.writeFile f $ B.runPut putRawFloats
    where putRawFloats = V.mapM_ putFloat32le xs
...
checkSquare xs = do
    let sz = V.length xs
    let singledim = floor . (sqrt :: Double -> Double) $ fromIntegral sz
